...
Základní nastavení aplikace je dostupné z hlavního menu. V této části lze upravit chování
notifikací, způsob zobrazení ovládacích prvků a povolit nebo zakázat použití mobilního připojení.

### Notifikace

Na Androidu 13 a novějším je nutné aplikaci povolit zobrazování notifikací. Aplikace je používá
především pro upozornění na začátek a konec nízkého tarifu (NT).

Pokud oprávnění pro notifikace není povoleno, upozornění se nemusí zobrazovat ani v případě, že jsou
časy HDO v aplikaci správně nastavené.

Klepnutím na příslušnou volbu v nastavení se otevře systémový dialog Androidu, ve kterém lze
oprávnění pro notifikace povolit nebo zakázat.

### Přesné alarmy

Na Androidu 12 a novějším může být pro přesné načasování upozornění nutné povolit oprávnění pro
přesné alarmy. Toto oprávnění pomáhá aplikaci zobrazit notifikaci na začátek nebo konec nízkého
tarifu ve správný čas.

Pokud přesné alarmy nejsou povolené, systém může upozornění odložit podle vlastních pravidel úspory
energie. To se může projevit tím, že se notifikace zobrazí později, než odpovídá nastavenému času
nízkého tarifu.

Klepnutím na volbu pro přesné alarmy se otevře systémové nastavení Androidu, kde lze toto oprávnění
upravit.

### Vynutit přesné notifikace

Volba **Vynutit přesné notifikace** je určena pro případy, kdy se upozornění zobrazují se zpožděním.
Může se to stát hlavně u zařízení s agresivní správou baterie nebo u telefonů s výrazně upravenou
nadstavbou systému.

Po zapnutí této volby se aplikace pokusí zajistit přesnější doručování upozornění. V některých
případech se může v systémové liště zobrazit ikona budíku. Jde o běžné chování Androidu při použití
přesněji plánovaných upozornění.

Tuto volbu není nutné zapínat vždy. Doporučuje se ji použít pouze tehdy, pokud se notifikace
zobrazují později, než odpovídá nastaveným časům nízkého tarifu.

### Zobrazení navigace

V části zobrazení navigace lze nastavit, jak budou v aplikaci dostupné nejpoužívanější ovládací
prvky.

- **Zobrazit plovoucí tlačítko pro přidání** – zobrazí kulaté plovoucí tlačítko pro přidávání nových
  položek. Tlačítko je umístěné v pravém dolním rohu nad seznamem záznamů. Pokud je volba vypnutá,
  tlačítko pro přidání se zobrazí pod aktuálním seznamem.
- **Zobrazit dolní panel rychlé navigace** – v režimu na výšku zobrazí ve spodní části obrazovky
  panel s ikonami pro rychlé přepínání mezi nejčastěji používanými částmi aplikace.
- **Zobrazit levý panel rychlé navigace** – v režimu na šířku zobrazí na levé straně panel s ikonami
  pro rychlé přepínání mezi nejčastěji používanými částmi aplikace.

Rychlá navigace obsahuje ikony pro přechod na **Přehled**, **Ceníky**, **Měsíční odečty**, **Odběrná
místa** a **Fakturaci**.

### Připojení

Volba pro připojení určuje, zda aplikace smí používat mobilní data pro funkce vyžadující internet.
Týká se především načítání časů HDO a přístupu ke Google Drive.

Pokud je použití mobilního připojení zakázáno, aplikace tyto online funkce provede až při dostupném
připojení přes Wi-Fi.

<p align="center">
  <img src="screenshots/nastaveni_2.png" width="300" />
  <img src="screenshots/nastaveni_1.png" width="300" />
</p>
